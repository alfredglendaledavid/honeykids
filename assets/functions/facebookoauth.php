...
function facebook_oauth_redirect()
{
	...
	$final_url = "https://www.facebook.com/dialog/oauth?client_id=" . urlencode($app_id) . "&redirect_uri=" . urlencode($redirect_url) . "&permission=" . $permission . "&scope=email";
...
function facebook_oauth_callback()
{
	...
	if(isset($_GET["code"]))
    {   
        $token_and_expire = file_get_contents("https://graph.facebook.com/oauth/access_token?client_id=" . get_field("facebook_app_id","option") . "&redirect_uri=". get_site_url() . "/wp-admin/admin-ajax.php?action=facebook_oauth_callback" . "&client_secret=" . get_field("facebook_app_secret","option") . "&code=" . $_GET["code"]);
        ...
        {   
        	...
        	if(username_exists($name))
			{
				...
				header('Location: ' . get_site_url(). '/my-profile/');
			}
			else
			{
